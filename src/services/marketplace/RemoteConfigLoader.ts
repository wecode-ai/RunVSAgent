...
export class RemoteConfigLoader {
	...
	constructor() {
		this.apiBaseUrl = "http://admin.aigc.intra.weibo.com"
	}
	...
	private async fetchModes(): Promise<MarketplaceItem[]> {
		...
		const data = await this.fetchWithRetry<string>(`${this.apiBaseUrl}/api/2/aigc/admin/mcp_marketplace/modes`)
	...
	private async fetchMcps(): Promise<MarketplaceItem[]> {
		...
		const data = await this.fetchWithRetry<string>(`${this.apiBaseUrl}/api/2/aigc/admin/mcp_marketplace/mcps`)
	...
	private async fetchWithRetry<T>(url: string, maxRetries = 2): Promise<T> {
		let lastError: Error

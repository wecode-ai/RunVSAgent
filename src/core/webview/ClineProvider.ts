...
import { convertAiModelSoure, WECODE_AUTO_DISPLAYNAME } from "../../wecoder/core/config/config"
import cloneDeep from "clone-deep"
import { markPinnedApiConfigs } from "../../wecoder/core/webview/ClienProvider"
	...
	implements vscode.WebviewViewProvider, TelemetryPropertiesProvider
{
	...
	public readonly latestAnnouncementId = "dec-12-2025-7-17" // Update for v3.20.0 announcement
	...
	*/
	...
	async activateProviderProfile(args: { name: string } | { id: string }) {
		...
		// 实时切换模型时, 将 taskId 传递给 apiConfiguration, 便于上报action日志
		providerSettings.taskId = this.getCurrentCline()?.taskId
	...
	async ensureMcpServersDirectoryExists(): Promise<string> {
		...
		if (process.platform === "win32") {
			...
			mcpServersDir = path.join(os.homedir(), "AppData", "Roaming", "WeCoder", "MCP")
		} else if (process.platform === "darwin") {
			// macOS: ~/Documents/Cline/MCP
			mcpServersDir = path.join(os.homedir(), "Documents", "WeCoder", "MCP")
		} else {
			// Linux: ~/.local/share/Cline/MCP
			mcpServersDir = path.join(os.homedir(), ".local", "share", "WeCoder", "MCP")
		}
		...
		try {
			...
			return path.join(os.homedir(), ".wecoder", "mcp")
		}
	...
	async getStateToPostToWebview() {
		const {
			...
			diagnosticsEnabled,
			disableVersionUpdateNotifications,
		} = await this.getState()
		...
		return {
			...
			shouldShowAnnouncement:
				!(disableVersionUpdateNotifications ?? false) && lastShownAnnouncementId !== this.latestAnnouncementId,
			...
			fuzzyMatchThreshold: fuzzyMatchThreshold ?? 0.9,
			mcpEnabled: mcpEnabled ?? true,
			enableMcpServerCreation: enableMcpServerCreation ?? false,
			alwaysApproveResubmit: alwaysApproveResubmit ?? false,
			requestDelaySeconds: requestDelaySeconds ?? 10,
			currentApiConfigName: currentApiConfigName ?? "default",
			currentAiModelSoure: currentAiModelSoure ?? WECODE_AUTO_DISPLAYNAME,
			externalRiskNoticeConfirmed: externalRiskNoticeConfirmed ?? false,
	...
	async getState() {
		...
		// 获取 wecoder 默认配置并赋值
		const currentApiConfigName = stateValues.currentApiConfigName ?? "wecode-auto"
		const apiConfig = await this.providerSettingsManager.getProfile({ name: currentApiConfigName })
		if (apiConfig) {
			const apiConfigDeep = cloneDeep(apiConfig)
			apiConfigDeep.id = ""
			providerSettings = apiConfigDeep
		}

		const configs = await this.providerSettingsManager.listConfig()

		const pinnedApiConfigs = markPinnedApiConfigs(configs, stateValues.pinnedApiConfigs)
		// 根据当前模式对应的模型id 实时转换为一键切换大模型显示的模型名字

		stateValues.currentAiModelSoure = providerSettings
			? convertAiModelSoure(providerSettings?.wecoderModelConfig || providerSettings.apiModelId)
			: ""
		...
		return {
			...
			fuzzyMatchThreshold: stateValues.fuzzyMatchThreshold ?? 0.9,
			...
			currentApiConfigName: stateValues.currentApiConfigName ?? currentApiConfigName,
			// 重置配置时, 开源版本当 stateValues.listApiConfigMeta 为空时, 赋值为 [], 内部版本需要赋值为系统默认值
			listApiConfigMeta: stateValues.listApiConfigMeta ?? configs,
			pinnedApiConfigs: pinnedApiConfigs,
			...
			currentAiModelSoure: stateValues.currentAiModelSoure ?? WECODE_AUTO_DISPLAYNAME,
			externalRiskNoticeConfirmed: stateValues.externalRiskNoticeConfirmed ?? false,
			disableVersionUpdateNotifications: stateValues.disableVersionUpdateNotifications ?? false,
		}
	}
	...
	async resetState() {
		...
		await updateWecoderDefaults(this)
		// 修复重置所有配置时, 无默认api配置的问题
		const defaultApiConfigMeta = await this.providerSettingsManager.listConfig()
		await this.context.globalState.update("listApiConfigMeta", defaultApiConfigMeta)

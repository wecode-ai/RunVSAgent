...
import { updateWecoderDefaults } from "../../wecoder/core/providerUtils"
import { handleWeCoderMessages } from "../../wecoder/core/webview/wecoderWebviewMessageHandler"
import { loadWeCodeModelConfig, updateWxternalRiskNoticeConfirmed } from "../../wecoder/core/config/providerManager"
...
import { checkAndPostCopilotConnectionStatus } from "../../wecoder/network/checkCopilotConnection"
...
export const webviewMessageHandler = async (
	...
	switch (message.type) {
		case "webviewDidLaunch":
			// copilot.weibo.com 连接检测
			await checkAndPostCopilotConnectionStatus(provider.postMessageToWebview.bind(provider))
			// Load custom modes first
			const customModes = await provider.customModesManager.getCustomModes()
			await updateGlobalState("customModes", customModes)
			// 判断如果wecoderModelConfig为空，则设置wecoder默认值
			const wecoderModelConfig = await provider.getGlobalState("wecoderModelConfig")
			if (!wecoderModelConfig) {
				await updateWecoderDefaults(provider)
			}
		...
		case "openProjectMcpSettings": {
			...
			const rooDir = path.join(workspaceFolder.uri.fsPath, ".wecode")
		...
		case "updateKnowledgeBases":
			if (message.values && Array.isArray(message.values)) {
				try {
					// 使用 context.globalState 存储知识库选择结果
					await provider.context.globalState.update("knowledgeBases", message.values)
					console.log(`[知识库] 保存成功: ${JSON.stringify(message.values)}`)

					// 同时更新全局MCP配置，添加或更新weibo-knowledge-mcp服务器
					try {
						const mcpHub = provider.getMcpHub()
						if (mcpHub) {
							// 准备weibo-knowledge-mcp的MCP配置
							const weiboRagDocsConfig = {
								command: "npx",
								args: [
									"-y",
									"--scope=@wb",
									"--registry=http://registry.npm.intra.weibo.cn",
									"@wb/weibo-knowledge-mcp@latest",
								],
								env: {
									SCOPES: message.values.join(","),
								},
								disabled: false,
								alwaysAllow: ["get-docs", "resolve-doc-name"],
							}

							// 更新全局MCP配置
							await mcpHub.updateServerConfig("weibo-knowledge-mcp", weiboRagDocsConfig, "global")
							await mcpHub.initializeMcpServers("global")
							console.log(
								`[知识库] 成功更新MCP服务器配置: weibo-knowledge-mcp，SCOPES: ${message.values.join(",")}`,
							)
						}
					} catch (mcpError) {
						provider.log(
							`Error updating MCP server config: ${JSON.stringify(mcpError, Object.getOwnPropertyNames(mcpError), 2)}`,
						)
						console.error("[知识库] 更新MCP服务器配置失败:", mcpError)
					}
				} catch (error) {
					provider.log(
						`Error updating knowledge bases: ${JSON.stringify(error, Object.getOwnPropertyNames(error), 2)}`,
					)
					console.error("[知识库] 保存失败:", error)
					vscode.window.showErrorMessage("保存知识库选择结果失败")
				}
			}
			break
		case "getKnowledgeBases":
			try {
				// 从 context.globalState 获取知识库选择结果
				const storedKnowledgeBases = provider.context.globalState.get("knowledgeBases")
				// 确保 knowledgeBases 是一个数组
				let knowledgeBases: string[] = Array.isArray(storedKnowledgeBases) ? storedKnowledgeBases : []
				console.log(`[知识库] 获取成功: ${JSON.stringify(knowledgeBases)}`)

				// 判断是否需要从接口获取知识库选项并过滤
				try {
					// 尝试从接口获取可用的知识库选项
					const response = await fetch("https://copilot.weibo.com/v2/ide/config")
					if (response.ok) {
						const data = await response.json()
						if (data && data.coder_knowledges && Array.isArray(data.coder_knowledges)) {
							// 获取所有可用的知识库名称
							const availableKnowledgeBases = data.coder_knowledges.map(
								(item: { name: string }) => item.name,
							)

							// 过滤掉不在可用列表中的知识库
							if (availableKnowledgeBases.length > 0) {
								knowledgeBases = knowledgeBases.filter((kb) => availableKnowledgeBases.includes(kb))
								console.log(`[知识库] 过滤后结果: ${JSON.stringify(knowledgeBases)}`)
							}
						}
					}
				} catch (fetchError) {
					// 如果获取失败，记录错误但继续使用原始知识库列表
					console.log(`[知识库] 获取可用选项失败，使用原始列表: ${fetchError}`)
				}

				// 发送结果给前端
				await provider.postMessageToWebview({
					type: "getKnowledgeBases",
					values: knowledgeBases,
				} as any) // 使用 as any 临时解决类型问题
			} catch (error) {
				provider.log(
					`Error getting knowledge bases: ${JSON.stringify(error, Object.getOwnPropertyNames(error), 2)}`,
				)
				vscode.window.showErrorMessage("获取知识库选择结果失败")
			}
			break
		case "getKnowledgeBaseOptions":
			try {
				// 从接口获取知识库选项
				const response = await fetch("https://copilot.weibo.com/v2/ide/config")
				if (!response.ok) {
				...
				const data = await response.json()
				console.log(`[知识库] 从接口获取选项成功: ${JSON.stringify(data)}`)

				// 转换数据格式并发送给前端
				const options = data.coder_knowledges.map((item: { name: string; desc: string }) => ({
					value: item.name,
					label: item.desc,
					type: "ITEM", // 使用 MultiDropdownOptionType.ITEM 的值
				...
				provider.log(
					`Error getting knowledge base options: ${JSON.stringify(error, Object.getOwnPropertyNames(error), 2)}`,
				)
				console.error("[知识库] 获取选项失败:", error)
				...
				vscode.window.showErrorMessage("获取知识库选项失败")
			}
			break
		...
		case "loadApiConfiguration":
			if (message.text) {
				try {
					const needUserConfirm = await loadWeCodeModelConfig(message, provider)
					// 外部模型需要用户进行弹窗确认风险提示
					if (needUserConfirm) {
						return
					}
		...
		case "loadApiConfigurationById":
			if (message.text) {
				try {
					await updateWxternalRiskNoticeConfirmed(message, provider)
		...
		default:
			await handleWeCoderMessages(provider, message)
	}
}

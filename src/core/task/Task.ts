...
import { ChunkRepetitionDetector } from "../../wecoder/core/tools/ChunkRepetitionDetector"
...
import { deduplicateConversationHistory, handleWeCodeProxyError } from "../../wecoder/core/clineUtils"
import { clearCache } from "../../wecoder/utils/cache/taskCache"
...
export class Task extends EventEmitter<ClineEvents> {
	...
	private chunkRepetitionDetector?: ChunkRepetitionDetector
	private duplicateContentDetected: boolean = false
	private duplicateContentRetryCount: number = 0
	...
	fuzzyMatchThreshold: number = 0.9
	...
	constructor({
		...
		// 将taskId赋值给apiConfiguration, 以便在上报action日志中使用
		...
		this.fuzzyMatchThreshold = fuzzyMatchThreshold ?? 0.9
		this.consecutiveMistakeLimit = consecutiveMistakeLimit ?? DEFAULT_CONSECUTIVE_MISTAKE_LIMIT
		this.providerRef = new WeakRef(provider)
		this.globalStoragePath = provider.context.globalStorageUri.fsPath

		// 将 taskId 透传至 DiffViewProvider, 上报action日志时, 需要使用 taskId
		this.diffViewProvider = new DiffViewProvider(this.cwd, this.taskId)
		...
		// 初始化chunk重复内容检测器
		this.chunkRepetitionDetector = new ChunkRepetitionDetector({
			maxBufferSize: 1000,
			enableTokenBasedDetection: true,
			debugMode: false,
		})
				...
				for await (const chunk of stream) {
					...
					switch (chunk.type) {
						...
						case "text": {
							...
							// 检测chunk是否包含重复内容
							if (this.chunkRepetitionDetector && !this.duplicateContentDetected) {
								const isRepetitive =
									this.chunkRepetitionDetector.detectChunkRepetition(assistantMessage)
								if (isRepetitive) {
									// 上报重复内容检测事件
									weCodeTelemetryClient.captureDuplicateContentDetected(
										this.taskId,
										this.duplicateContentRetryCount,
										"chunk_repetition",
									)
								}
							}
	...
	public async *attemptApiRequest(retryAttempt: number = 0): ApiStream {
		...
		if (contextTokens) {
			...
			deduplicateConversationHistory(this.apiConversationHistory)
		}

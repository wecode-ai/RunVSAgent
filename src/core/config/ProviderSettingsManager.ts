...
import { autoConfig, isExternalModel, merge_config } from "../../wecoder/core/config/config"
...
export class ProviderSettingsManager {
	private static readonly SCOPE_PREFIX = "wecoder_config_"
	...
	public async initialize() {
		try {
			return await this.lock(async () => {
				...
				if (!providerProfiles) {
					// 默认配置改为 autoConfig
					await this.store(autoConfig)
					return
				}

				const needsUpdate = merge_config(providerProfiles)
				let isDirty = false
	...
	public async store(providerProfiles: ProviderProfiles) {
		try {

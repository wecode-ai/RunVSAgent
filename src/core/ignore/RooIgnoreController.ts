...
export class RooIgnoreController {
	...
	private setupFileWatcher(): void {
		const rooignorePattern = new vscode.RelativePattern(this.cwd, ".wecodeignore")
	...
	private async loadRooIgnore(): Promise<void> {
		try {
			...
			this.ignoreInstance = ignore()
			const ignorePath = path.join(this.cwd, ".wecodeignore")
			if (await fileExistsAtPath(ignorePath)) {
				const content = await fs.readFile(ignorePath, "utf8")
				this.rooIgnoreContent = content
				this.ignoreInstance.add(content)
				this.ignoreInstance.add(".wecodeignore")
			} else {
				this.rooIgnoreContent = undefined
			}
		} catch (error) {
			// Should never happen: reading file failed even though it exists
			console.error("Unexpected error loading .wecodeignore:", error)
		}
	}
	...
	getInstructions(): string | undefined {
		...
		return `# .wecodeignore\n\n(The following is provided by a root-level .wecodeignore file where the user has specified files and directories that should not be accessed. When using list_files, you'll notice a ${LOCK_TEXT_SYMBOL} next to files that are blocked. Attempting to access the file's contents e.g. through read_file will result in an error.)\n\n${this.rooIgnoreContent}\n.wecodeignore`
	}
}

...
export async function loadRuleFiles(cwd: string): Promise<string> {
	...
	const ruleFiles = [".roorules", ".wecoderules", ".clinerules", ".windsurfrules"]
...
export async function addCustomInstructions(
	...
	if (mode) {
		...
		if (modeRules.length > 0) {
			...
			const wecodeModeRuleFile = `.wecoderules-${mode}`
			modeRuleContent = await safeReadFile(path.join(cwd, wecodeModeRuleFile))
			if (modeRuleContent) {
				usedRuleFile = wecodeModeRuleFile
			} else {
	...
	if (modeRuleContent && modeRuleContent.trim()) {
		if (usedRuleFile.includes(path.join(".wecode", `rules-${mode}`))) {

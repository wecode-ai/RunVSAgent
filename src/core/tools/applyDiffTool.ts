...
import { TelemetryService, weCodeTelemetryClient } from "@roo-code/telemetry"
	...
	try {
		if (block.partial) {
			...
			if (!diffResult.success) {
				...
				// diff 成功的也上报日志, 用于统计分析diff失败的占比
				weCodeTelemetryClient.captureDiffApplicationSuccess(cline.taskId, 0)
			}

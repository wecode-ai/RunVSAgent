...
import { ModelQuota } from "@/wecoder/components/chat/ModelQuota"
...
const TaskHeader = ({
	...
	const isWeCodeModel = useMemo(() => {
		return apiConfiguration?.apiProvider === "wecoder"
					...
					<div className={`w-full flex flex-row items-center gap-1 h-auto`}>
						...
						{isWeCodeModel && (
							<div className={`flex ${windowWidth < 270 ? "flex-col" : "flex-row"} gap-1 `}>
								<ModelQuota modelType="claude" isEnabled={true} showProgressBar={false} />
							</div>
						)}
						...
						<div className="flex flex-col gap-1">
							...
							{isWeCodeModel && (
								<div className={`flex ${windowWidth < 270 ? "flex-col" : "flex-row"} gap-1 h-[20px]`}>
									<ModelQuota modelType="claude" isEnabled={true} />
								</div>
							)}
